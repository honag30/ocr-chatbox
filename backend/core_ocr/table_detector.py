...
def format_table_to_markdown(table_matrix: list[list[str]]) -> str:
    ...
    Tự động lọc bỏ các cột hoàn toàn rỗng để giữ định dạng bảng gọn gàng.
    """
    if not table_matrix:
        return ""
    ...
    # 1. Nhận diện các cột có dữ liệu (ít nhất 1 ô không rỗng)
    valid_col_indices = [
        c for c in range(num_cols)
        if any(norm_matrix[r][c].strip() for r in range(len(norm_matrix)))
    ]

    # Nếu tất cả cột đều rỗng -> trả về rỗng
    if not valid_col_indices:
        return ""

    # 2. Rút gọn ma trận chỉ giữ lại các cột có dữ liệu
    pruned_matrix = []
    for row in norm_matrix:
        pruned_matrix.append([row[c] for c in valid_col_indices])

    pruned_num_cols = len(valid_col_indices)
    col_widths = [3] * pruned_num_cols
    for row in pruned_matrix:
        for c_idx, cell in enumerate(row):
            col_widths[c_idx] = max(col_widths[c_idx], len(cell))

    header = pruned_matrix[0]
    header_line = "| " + " | ".join(header[i].ljust(col_widths[i]) for i in range(pruned_num_cols)) + " |"
    sep_line = "| " + " | ".join("-" * col_widths[i] for i in range(pruned_num_cols)) + " |"

    body_lines = []
    for row in pruned_matrix[1:]:
        line = "| " + " | ".join(row[i].ljust(col_widths[i]) for i in range(pruned_num_cols)) + " |"
